...
const List=({navigation,route})=> {
    ...
    const renderTodo=({item}:any)=>{
        ...
        const addFavorite= async()=>{
            const doc=addDoc(collection(FIRESTORE_DB,'Favorite'),{id:params,title:title,hinhanh:hinhanh}).then(()=>alert("Thêm thành công"))
        }
        ...
        return(
            <View style={styles.todoContainer}>
                ...
                <View style={{flex:2,flexDirection:"row",alignItems:"flex-end",justifyContent:"center",marginLeft:13}}>
                    <View style={{flexDirection:"row",paddingRight:15}}>
                    <Ionicons name="trash-bin-outline" size={24} color="red" onPress={deletitem}/>
                </View>
                <View style={{flexDirection:"row"}}> 
                    <TouchableOpacity style={{paddingRight:10}} onPress={addFavorite}>
  ...
  return (
    ...
    <View style={styles.container}> 
        <View>
           <Button onPress={()=>{navigation.navigate("AddService")}} title='Thêm phụ kiện' />
        </View>
